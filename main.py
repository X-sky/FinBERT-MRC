...
def training(opt):
    ...
    # local test - load features from file
    # temp_feature_path = os.path.abspath("./temp/train_features.pkl")
    # if os.path.exists(temp_feature_path):
    #     logger.info(f"loading features from {temp_feature_path}")
    #     train_features = torch.load(temp_feature_path)
    # else:
    #     train_features = convert_examples_to_features(
    #         train_examples, opt.max_seq_len, opt.bert_dir, ent2id
    #     )[0]
    #     torch.save(train_features, temp_feature_path)
    # extract features
    train_features = convert_examples_to_features(
        train_examples, opt.max_seq_len, opt.bert_dir, ent2id
    )[0]
    train_dataset = NERDataset(
        train_features, "train", use_type_embed=opt.use_type_embed
    )

    model = build_model(
        opt.bert_dir, dropout_prob=opt.dropout_prob, loss_type=opt.loss_type
    )
...
def validate(opt):
    ...
    dev_features, dev_callback_info = convert_examples_to_features(
        dev_examples, opt.max_seq_len, opt.bert_dir, ent2id
    )
    ...
    device = torch.device(
        "cpu"
        if opt.gpu_ids.split(",")[0] == "-1"
        else "cuda:" + opt.gpu_ids.split(",")[0]
    )
...
if __name__ == "__main__":
    ...
    relative_dir_arg = ["mid_data_dir", "output_dir", "raw_data_dir", "bert_dir"]
    for dir in relative_dir_arg:
        setattr(args, dir, os.path.abspath(getattr(args, dir)))
